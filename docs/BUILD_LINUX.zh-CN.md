...
发行物采用“PyInstaller onedir + AppImage 外壳”。运行时保留命令台，后端只监听 loopback，并自动打开系统浏览器。AppImage 内只读；下载、日志、缓存、凭据路径和新增制品全部写入外部用户数据目录。
...
构建脚本会验证当前 D Viewer checksums、控制中心测试、冻结程序自检、实际冻结任务子进程、编排器内部入口、HTTP 端点和路径穿越拒绝，再生成 SHA256。脚本内置本次已审查 appimagetool 与 type-2 runtime 的 SHA256；在线文件若变化会安全失败。正式长期发布仍应把这两个工具固定到内部制品库。
